...
def string_to_list(value):
  ...
  return [i.split(',') for i in val]

def create_points_list(value):
  result = [[], []]
  result_steps = [[], []]

  for inx, wire in enumerate(value):
    x = 0
    y = 0
    ...
    for turn in wire:
      ...
      for i in range(int(turn[1:])):
        x += 1*x_multiplicator
        y += 1*y_multiplicator
        result[inx].append((x, y))
  
  return result

def matching_points_list(value):
  return list(set(value[0]) & set(value[1]))

def compute_answer(value):
  points = create_points_list(string_to_list(value))
  matching_points = matching_points_list(points)

  matching_distances = [points[0].index(point) + 1 + points[1].index(point) + 1 for point in matching_points]

  return min(map(lambda x: abs(x[0]) + abs(x[1]), matching_points)), min(matching_distances)

print(compute_answer(input))